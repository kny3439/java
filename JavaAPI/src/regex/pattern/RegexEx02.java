...
public class RegexEx02 {
	...
	public static void main(String[] args) {
		...
		String info="30세/홍길동/강남구/02-123-1234/010-1234-5678/aaa@example.com";
		
		String pattern1="\\d{2,3}-\\d{3,4}-\\d{4}";
		
		Pattern p=Pattern.compile(pattern1); // 정규표현 문자열을 넣는다.
		Matcher m=p.matcher(info); // 적용 할 문자열을 넣는다.
...
//		}
		...
		while(m.find()) { // 1번 수행 시 앞에서 한 번 찾음
			System.out.println("정규표현식에 해당하는 문자를 찾음");
			System.out.println(m.group());
			System.out.println("시작 위치: "+m.start());
			System.out.println("끝 위치: "+m.end());
